...
将扩展打包为**单个 Windows 安装程序（setup.exe）**，用户双击即可完成安装（无需手动复制文件、无需任何外部依赖）。安装到**系统级** CEP 扩展目录，使 After Effects 能正确识别并加载扩展，**需要管理员权限**（运行时会弹出 UAC 提权请求）。
...
- 安装动作：把文件写到**系统级** CEP 扩展目录（`C:\Program Files (x86)\Common Files\Adobe\CEP\extensions\AeLocalToolkit`），并写入 `HKLM\Software\Adobe\CSXS.9~14\PlayerDebugMode=1`（覆盖 AE 2020–2025，启用未签名扩展加载，对所有用户生效）。**需要管理员权限**（运行安装器时会弹出 UAC 提权）。
- 附带 `uninstall.exe`，可在「添加/删除程序」中卸载（同样需管理员权限）。
...
2. 阅读许可协议，点击「下一步」。
3. 确认安装路径（默认已填好系统级 CEP 目录 `C:\Program Files (x86)\Common Files\Adobe\CEP\extensions\AeLocalToolkit`，可点击「浏览…」自定义），点击「安装」。若弹出用户账户控制（UAC），请点击「是」授权。
...
2. 移动到**系统级** CEP 扩展目录（用户级目录 After Effects 不识别，面板不会显示，只能用系统级）：`/Library/Application Support/Adobe/CEP/extensions/`（需管理员权限，如 `sudo cp -r AeLocalToolkit /Library/Application\ Support/Adobe/CEP/extensions/`）。
